...
export async function createTask(req: AuthRequest, res: Response) {
  ...
  const task = await prisma.task.create({ data: { title, description, dueDate: dueDate ? new Date(dueDate) : null, userId: req.userId! } })
...
export async function updateTask(req: AuthRequest, res: Response) {
  ...
  const updated = await prisma.task.update({ where: { id }, data: { title, description, completed, dueDate: dueDate ? new Date(dueDate) : null } })
  res.json(updated)
}
